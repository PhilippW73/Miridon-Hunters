...
export default {
  aimedAttack: function(who, target) {
    who.curStats.speed_point = who.curStats.speed_point - 3;
    let comment = "";
    if(target.Defensive = "Block"){
      var damage = Math.max((who.fullStats.strength_point - target.fullStats.strength_point + Math.floor(Math.random()*6)+Math.floor(Math.random()*6)+2)+3, 0);
