  ...
  gunAttack: function(who, target) {
    ...
    if(target.Defensive = "Block"){
      ...
      if(who.Movement != "Charge") {
          const comment = (who.fullStats.character_name + " charged.")
          damage = damage + 2;
      }
      target.curStats.hit_point =  target.curStats.hit_point - damage;
      const comment = (target.fullStats.character_name + " blocked "+who.fullStats.character_name+"'s attack and got dealt " + damage + " damage.");
    } else if(target.Defensive = "Dodge" && (target.fullStats.speed_point - who.fullStats.speed_point + Math.floor(Math.random()*6)+1 > 4)){
      //opposition dodges
      const comment = (who.fullStats.character_name + " missed due to "+target.fullStats.character_name+" dodging.");
    } else {
      //no defense
      ...
      const comment = (who.fullStats.character_name+"'s attack dealt " + damage + " damage to "+target.fullStats.character_name+".");
    }
